    ...
    <div class="span6">
        <div class="ranking-box-container">
            <div class="ranking-header">
                ...
                <div class="select-kups-ranking">
                    <form action="" method="post">
                        <select name="kup_uuid" id="select-kup-uuid">
                            <option value="">Général</option>
                            <?php foreach ($kupsData as $kup) : ?>
                            <option value="<?php echo $kup['uuid'] ?>">
                                <?php echo $kup['name'] ?>
                            </option>
                            <?php endforeach ?>
                        </select>
        ...
        <div class="ranking-box-container">
            ...
            <div class="ranking-content" id="ranking-individual-honor"></div>
        </div>
    </div>
    <div class="span6">
        <div class="ranking-box-container">
...
<script type="text/javascript">
    $(function () {
        ...
        $('#select-kup-uuid').change(function() {
            loadRanking(0, 11);
        });

        $('#ranking-individual-honor').loadContent({
            'url':'<?php echo url_for(array(
                                           'module'  => 'facebook_ligue1_2012',
                                           'action'  => 'rankingIndividualHonorBox'
                                      )); ?>',
            'method':'POST',
            data:{}
        }, "resizeCanvas()");
    ...
    function loadRanking(offset, batchSize) {
        ...
        var kup_uuid = $('#select-kup-uuid').val();

        $('#loading-ranking-individual-content').loadContent({
            'url':'<?php echo url_for(array(
                                           'module'  => 'facebook_ligue1_2012',
                                           'action'  => 'rankingBoxIndividual'
                                      )); ?>',
            'method':'POST',
            data:{
                'access_token':'<?php echo $sf_data->getRaw('access_token') ?>',
